Ext.define("lppExt.form.Factory", {
    ...
    _onUpdateClick: function (item) {
        ...
        if (checkedModels.length) {
            id = checkedModels[0].raw.id || checkedModels[0].raw.ID;
        }
        else {
            id = selectedModels[0].raw.id || selectedModels[0].raw.ID;
        }
        Ext.Ajax.request({
            url: item.uUrl,
            method: "GET",
            ...
            success: function (response) {
                ...
                for (i = 0; i < inputs.length; ++i) {
                    if (inputs[i].xtype === 'lppTreeList') {
                        var selectedUrl = inputs[i].selectedUrl;
                        inputs[i].selectedUrl = "";
                        lppExt.util.Common.syncRequest({
                            ...
                            success: function (response, eOpt) {
                                ...
                                inputs[i].selectedItems = items;
                            }
                        });
                    }
                    else if (Ext.isFunction(inputs[i].getValue)) {
                        ...
                        if (result.items.length === 1) {
                            for (var itemP in result.items[0]) {
                                if (inputs[i].name === itemP) {
                                    inputs[i].value = Ext.util.Format.htmlDecode(result.items[0][itemP]);
                                }
                            }
                        }
                        else {
                            for (j = 0; j < result.items.length; ++j) {
                                if (inputs[i].name === result.items[j].name) {
                                    for (p in result.items[j]) {
                                        inputs[i][p] = Ext.util.Format.htmlDecode(result.items[j][p]);
                                    }
                                }
                            }
                        }
